...
module KeyInputHandler =
    ...
    let ProcessInput (args : KeyPressEventArgs) : InputCommand option =
        ...
        | Gdk.Key.Tab -> Some InputCommand.Tab
        | Gdk.Key.BackSpace -> Some InputCommand.Backspace
        | Gdk.Key.Insert -> Some InputCommand.Insert
        | Gdk.Key.Delete -> Some InputCommand.Delete
        | Gdk.Key.Left -> Some InputCommand.Left
        | Gdk.Key.Right -> Some InputCommand.Right
        | Gdk.Key.Up -> Some InputCommand.Up
        | Gdk.Key.Down -> Some InputCommand.Down
        | Gdk.Key.Page_Up -> Some InputCommand.PageUp
        | Gdk.Key.Page_Down -> Some InputCommand.PageDown
        | Gdk.Key.Home -> Some InputCommand.Home
        | Gdk.Key.End -> Some InputCommand.End
        | Gdk.Key.Escape -> Some InputCommand.Escape
        | key when key >= Gdk.Key.F1 && key <= Gdk.Key.F24 ->
            InputCommand.Function (byte (key - Gdk.Key.F1) + 1uy, InputModifiers.None (* KeyboardModifiers() *)) |> Some
        //| Key.Pause
        | _ ->
            let k = uint32 args.Event.Key
            let c = char (Keyval.ToUnicode k)
            InputCommand.Character c |> Some
            //match charFromKey args.Event.Key with
...
//    //let getCaretLine (editor: MonoDevelop.Ide.Editor.TextEditor) =
//    //    let line =
//    //        editor.CaretLine
//    //        |> editor.GetLine
...
//    override x.KeyPress (descriptor:MonoDevelop.Ide.Editor.Extension.KeyDescriptor) =
...
type ConsoleSession((*ssh : SshClient,*) display : Display,
    ...
    static let DebugPrint (host : string) (out : bool) (data : byte[]) : unit =
        ...
        let formatChar (c : byte) =
            if c >= 0x20uy && c < 0x7Fuy then
                ...
                String.Format("   {0}", c)
        String.Join(Environment.NewLine, header :: (Seq.map formatChar data |> List.ofSeq)) |> Console.WriteLine
    ...
    //let mutable stream : Stream (*ShellStream *)= null
